...
3. **VC退室時の自動削除**: 退室・別VCへの移動時に入室通知メッセージを自動削除(`DELETE_ON_LEAVE`で切替可能)
4. **削除検知**: 自己紹介チャンネルでメッセージが削除されるとDBレコードとロールを同期して剥奪
5. **週次リマインダー**: 毎週月曜10:00に未投稿者へ通知
6. **自動ロール付与**: 自己紹介投稿時に「自己紹介済み」ロールを自動付与
7. **スラッシュコマンド**: `/profilebot` で手動リマインダー実行、`/profile @user` で指定ユーザーの自己紹介を表示
