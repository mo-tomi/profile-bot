...
async def report(
    ...
    # 先に応答しないとタイムアウトすることがあるため、最初に応答する
    await interaction.response.defer(ephemeral=True)

    if violated_rule.value == "その他" and not details:
        await interaction.followup.send("「その他」を選んだ場合は、具体的な状況を `details` に入力してください。", ephemeral=True)
        return
        
    if REPORT_CHANNEL_ID == 0:
        await interaction.followup.send("現在、ボットが設定中のため通報機能を利用できません。", ephemeral=True)
        return
    ...
    try:
        ...
        if not report_channel:
            await interaction.followup.send("報告用チャンネルが見つかりません。管理者に設定を確認するよう伝えてください。", ephemeral=True)
            return
        ...
        await interaction.followup.send("通報を受け付けました。ご協力ありがとうございます。", ephemeral=True)
